...
async function main() {
  ...
  // Load monorepo root .env for shared credentials (Hostinger, Cloudflare)
  // __dirname is packages/core/scripts, so root is ../../../
  ...
  const { UPLOAD_HOST, UPLOAD_PORT, UPLOAD_USER, UPLOAD_PASS, UPLOAD_DIR, PROJECT_SLUG } = process.env;
  ...
  const mkdirCmd = `sshpass -p '${UPLOAD_PASS}' ssh -p ${UPLOAD_PORT} -o StrictHostKeyChecking=no ${UPLOAD_USER}@${UPLOAD_HOST} "mkdir -p ${TARGET_DIR}/books ${TARGET_DIR}/audiobook ${TARGET_DIR}/pdf"`;
  if (!run(mkdirCmd)) {
  ...
  if (require('fs').existsSync(localBooks)) {
    ...
    const rsyncBooks = `sshpass -p '${UPLOAD_PASS}' rsync -avz -e "ssh -p ${UPLOAD_PORT} -o StrictHostKeyChecking=no" ${localBooks} ${UPLOAD_USER}@${UPLOAD_HOST}:${TARGET_DIR}/books/`;
    run(rsyncBooks);
  }
  ...
  if (require('fs').existsSync(localAudio)) {
    ...
    const rsyncAudio = `sshpass -p '${UPLOAD_PASS}' rsync -avz -e "ssh -p ${UPLOAD_PORT} -o StrictHostKeyChecking=no" ${localAudio} ${UPLOAD_USER}@${UPLOAD_HOST}:${TARGET_DIR}/audiobook/`;
    run(rsyncAudio);
  }
  ...
  if (require('fs').existsSync(localPdf)) {
    ...
    const rsyncPdf = `sshpass -p '${UPLOAD_PASS}' rsync -avz -e "ssh -p ${UPLOAD_PORT} -o StrictHostKeyChecking=no" ${localPdf} ${UPLOAD_USER}@${UPLOAD_HOST}:${TARGET_DIR}/pdf/`;
    run(rsyncPdf);
  }
